#include <DX11Util/Dx11EffectGUICanvas.h>


#define SAFE_RELEASE(x)  { if(x) { (x)->Release(); (x)=NULL; } }	// 解放マクロ

Dx11EffectGUICanvas::Dx11EffectGUICanvas(void)
{
}
...
HRESULT Dx11EffectGUICanvas::Setup(Dx11Context* _context)
{
	HRESULT hr; 
...
UINT flagCompile = D3D10_SHADER_ENABLE_STRICTNESS | D3D10_SHADER_PACK_MATRIX_COLUMN_MAJOR;
	...
	// for view
	cBufferDesc.ByteWidth      = sizeof(EffectGUICanvasInfo);
...
HRESULT Dx11EffectGUICanvas::Update(Dx11Context* _context, Dx11ObjectGUI* obj, EffectGUICanvasInfo* param)
{
	...
	// データ書き込み
	CopyMemory(MappedResource.pData, param, sizeof(EffectGUICanvasInfo));
	// マップ解除
...
HRESULT Dx11EffectGUICanvas::Term(Dx11Context* context)
{
