...
Dx11EffectHDRMix::Dx11EffectHDRMix(void)
{
}
...
HRESULT Dx11EffectHDRMix::Setup(Dx11Context* _context)
{
	HRESULT hr; 
...
UINT flagCompile = D3D10_SHADER_ENABLE_STRICTNESS | D3D10_SHADER_PACK_MATRIX_COLUMN_MAJOR;
	...
	// for view
	cBufferDesc.ByteWidth      = sizeof(EffectHDRMixInfo);
...
HRESULT Dx11EffectHDRMix::Update(Dx11Context* _context, Dx11Object* base, Dx11ObjectTargetTexture** bright, Dx11ObjectGUI* hdr_src, int bright_num,  EffectHDRMixInfo* param)
{
	HRESULT hr;
	unsigned int max; 
	...
	// データ書き込み
	CopyMemory(MappedResource.pData, param, sizeof(EffectHDRMixInfo));
	// マップ解除
	...
	base->GetVertexBuf(0, &vb); 
	base->GetIndexBuf(0, &ib); 

	base->GetVertexNum(0, &vn); 
	base->GetIndexNum(0, &in); 
	...
	srv = new ID3D11ShaderResourceView*[bright_num+2]; 
	base->GetTexture(0, &(srv[0])); 
	hdr_src->GetTexture(0, &(srv[1])); 
	for(int i=0;i<bright_num;i++) {
		bright[i]->GetTexture(0, &(srv[i+2])); 
	}

	context->PSSetShaderResources(
	    0,              // 設定する最初のスロット番号
	    bright_num+2,              // 設定するシェーダ・リソース・ビューの数
...
HRESULT Dx11EffectHDRMix::Term(Dx11Context* _context)
{
