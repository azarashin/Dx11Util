...
Dx11EffectHDRBright::Dx11EffectHDRBright(void)
{
}
...
HRESULT Dx11EffectHDRBright::Setup(Dx11Context* _context)
{
	HRESULT hr; 
...
UINT flagCompile = D3D10_SHADER_ENABLE_STRICTNESS | D3D10_SHADER_PACK_MATRIX_COLUMN_MAJOR;
	...
	hr = D3DCompileFromFileWrapper(
			L"shader\\hdr_bright.hlsl",  // file name
	...
	// for view
	cBufferDesc.ByteWidth      = sizeof(EffectHDRBrightInfo);
...
HRESULT Dx11EffectHDRBright::Update(Dx11Context* _context, Dx11ObjectGUI* obj, Dx11ObjectGUI* hdr_src, EffectHDRBrightInfo* param)
{
	HRESULT hr;
	unsigned int max; 
	...
	// データ書き込み
	CopyMemory(MappedResource.pData, param, sizeof(EffectHDRBrightInfo));
	// マップ解除
...
HRESULT Dx11EffectHDRBright::Term(Dx11Context* _context)
{
