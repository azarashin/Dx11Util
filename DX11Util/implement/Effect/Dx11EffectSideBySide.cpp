...
Dx11EffectSideBySide::Dx11EffectSideBySide(int _width, int _height, const EffectSideBySideInfo* param)
{
	width = _width; 
	height = _height; 

	memcpy(&hmd_param, param, sizeof(EffectSideBySideInfo)); 
}


Dx11EffectSideBySide::~Dx11EffectSideBySide(void)
{
}



HRESULT Dx11EffectSideBySide::Setup(Dx11Context* _context)
{
	HRESULT hr; 
...
UINT flagCompile = D3D10_SHADER_ENABLE_STRICTNESS | D3D10_SHADER_PACK_MATRIX_COLUMN_MAJOR;
	...
	// for view
	cBufferDesc.ByteWidth      = sizeof(EffectSideBySideInfo);
...
HRESULT Dx11EffectSideBySide::Update(Dx11Context* _context, Dx11Scene* scene, Dx11CameraSideBySide* camera, Dx11LensSideBySide* lens)
{
	...
	EffectSideBySideInfo iparam; 
	param = &iparam; 

	memcpy(param, &hmd_param, sizeof(EffectSideBySideInfo)); 
	...
	// データ書き込み
	param->left = -0.5f; 
	CopyMemory(MappedResource.pData, param, sizeof(EffectSideBySideInfo ));
	// マップ解除
	...
	for(unsigned int i=0;i<max;i++)
	{
		...
		left->GetVertexNum(i, &vn); 
		left->GetIndexNum(i, &in); 
...
//		pd3dDevice->SetTexture(0, NULL);
	...
	for(unsigned int i=0;i<max;i++)
	{
		...
		right->GetVertexNum(i, &vn); 
		right->GetIndexNum(i, &in); 
...
HRESULT Dx11EffectSideBySide::Term(Dx11Context* _context)
{
